...
class MemN2N(object):
    ...
    def __init__(self, batch_size, vocab_size, sentence_size, memory_size, embedding_size, output_size,
        hops=3,
        max_grad_norm=40.0,
        nonlin=None,
    ...
    def _build_inputs(self):
        ...
        self._answers = tf.placeholder(tf.int32, [None, self._output_size], name="answers")
        self._lr = tf.placeholder(tf.float32, [], name="learning_rate")

    def _build_vars(self):
        with tf.variable_scope(self._name):
#            nil_word_slot = tf.zeros([1, self._embedding_size]) # (1 * 20) size of nil_word_slot
            A = tf.concat(values=[self._init([self._vocab_size, self._embedding_size]) ],axis=0)
            C = tf.concat(values=[self._init([self._vocab_size, self._embedding_size]) ], axis=0)
            L = tf.concat(values=[self._init([self._output_size, self._embedding_size]) ], axis=0)
            self.A_1 = tf.Variable(A, name="A")
            self.L_1 = tf.Variable(L, name="L")
            self.C = []
        ...
        with tf.variable_scope(self._name):
            ...
            for hopn in range(self._hops):
                if hopn == 0:
                    ...
                    print("\n self.A_1",self.A_1,"\n m_emb_A",m_emb_A,"\n stories",stories,"\nm_emb_A * self._encoding",m_emb_A * self._encoding)
                ...
                print("\n u[-1]",u[-1])
                print("\n tf.expand_dims(u[-1], -1)", tf.expand_dims(u[-1], -1))
                u_temp = tf.transpose(tf.expand_dims(u[-1], -1), [0, 2, 1])
                print("\nu_temp",u_temp)
                print("\nm_A * u_temp",m_A * u_temp)
                ...
                print("\n c_temp probs_temp c_temp * probs_temp",c_temp,probs_temp,c_temp * probs_temp)
            ...
            with tf.variable_scope('hop_{}'.format(self._hops)):
               return tf.matmul(u_k, tf.transpose(self.L_1, [1,0]))
    ...
    def batch_fit(self, stories, queries, answers, learning_rate):
        ...
        """
#        print("\n query_classes", query_classes.shape)
